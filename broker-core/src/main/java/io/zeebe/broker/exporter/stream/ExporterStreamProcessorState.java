...
public class ExporterStreamProcessorState extends StateController {
  ...
  private final ZbString exporterId;
  private final ZbLong position;
  private final ColumnFamily<ZbString, ZbLong> exporterPositionColumnFamily;

  public ExporterStreamProcessorState(ZeebeDb<ExporterColumnFamily> zeebeDb) {
    exporterId = new ZbString();
    position = new ZbLong();
    exporterPositionColumnFamily =
        zeebeDb.createColumnFamily(ExporterColumnFamily.DEFAULT, exporterId, position);
  }
  ...
  private long getPosition() {
    final ZbLong zbLong = exporterPositionColumnFamily.get(exporterId);
